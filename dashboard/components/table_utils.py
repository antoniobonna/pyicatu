...
def display_dataframe_with_config(df, use_container_width=True, column_config=None):
    ...
    st.dataframe(data=df, use_container_width=use_container_width, column_config=column_config)
    ...
    for i, col in enumerate(df.columns):
        ...
        if pd.api.types.is_numeric_dtype(df[col]):
            ...
            filtered_df = filtered_df[
                (filtered_df[col] >= filter_values[0]) & (filtered_df[col] <= filter_values[1])
            ]
        ...
        # For categorical columns, create a multi-select dropdown
            ...
            selected = st.multiselect(
                f"Filter by {col}", options=options, default=options, key=f"{key_prefix}_{i}"
            )
        ...
        # For date columns, create a date range selector
            ...
            filter_dates = st.date_input(
                f"Filter by {col}", value=(min_date, max_date), key=f"{key_prefix}_{i}"
            )
...
def format_currency(value, prefix="R$", decimal_places=2):
    ...
    return (
        f"{prefix} {value:,.{decimal_places}f}".replace(",", "X")
        .replace(".", ",")
        .replace("X", ".")
    )

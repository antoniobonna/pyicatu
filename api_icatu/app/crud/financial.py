...
class TickerService:
    ...
    @staticmethod
    def create_ticker(
        db: Session, ticker_nm: str, ticker_type_nm: str, annual_tax: float
    ) -> Ticker:
        """
        Create a new ticker.
    ...
    @staticmethod
    def create_ticker_type(db: Session, ticker_type_nm: str) -> TickerType:
        """
        ...
        Returns:
            Created TickerType object
        """
        # Generate unique ID for the ticker type
        ticker_type_id = (generate_ticker_id(ticker_type_nm),)

        # Create TickerType object
        ticker_type = TickerType(
            ticker_type_id=ticker_type_id, ticker_type_nm=ticker_type_nm, is_src=True
        )

        # Add to database and commit
        db.add(ticker_type)
        db.commit()
        db.refresh(ticker_type)
